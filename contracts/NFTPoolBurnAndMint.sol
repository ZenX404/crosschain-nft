...
// 目的链的 NFTPool（NFT池） BurnAndMint用于销毁和铸造Wrapped NFT（包装NFT）
...
contract NFTPoolBurnAndMint is CCIPReceiver, OwnerIsCreator {
    ...
    function burnAndSendNFT(
        ...
        address receiver // 目标链的receiver，也就是目标链的合约地址
            ...
            // 将包装NFT转移到当前合约中（NFTPool BurnAndMint），用于销毁包装NFT
            wnft.transferFrom(msg.sender, address(this), tokenId);

            // burn the wnpf before send to ccip
            // 销毁包装NFT    
            // 每一个源NFT和对应的包装NFT共用同一个tokenId  
            wnft.burn(tokenId);
    ...
    function _ccipReceive(
        Client.Any2EVMMessage memory any2EvmMessage // 接收到的信息被封装到了这个对象中
    )
        internal
        override
    {
